...
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
	<script type="text/javascript" >
		...
		function check1(){
			var dengluming=get("dengluming");
			var nicheng=get("nicheng");
			var mima=get("mima");
			var youxiang=get("youxiang");	
			
		
					
			if(isNull(dengluming)){
				alert('登录名不能为空');
				return false;
			}
			if(isNull(nicheng)){
...
function showProvice(n)//显示省份
{
  ...
  <%UserInfo user = (UserInfo)session.getAttribute("u"); %>
    ...
    <h1 style="font-weight: normal;"><br></h1><h1 style="font-weight: normal;">&nbsp; <strong><font size="6" color="#425e8c">用户信息修改</font></strong></h1><br/><br/>
   	
   	<form action="user!modifyUser.action?user.userId=${u.userId}" method="post"> 
   		...
   		<tr><td style="text-align: left; width: 50px;">用户名</td>
   		<td>${u.userName}</td></tr>
   		<tr><td>&nbsp;</td></tr>
   		<tr><td style="text-align: left; width: 50px;">昵称</td>
   		<td><input type="text" name="user.userNickname" id="nicheng"  style="height: 20px" value="${u.userNickname}"/></td></tr>
   		<tr><td>&nbsp;</td></tr>
   		<tr><td style="text-align: left; width: 50px;">邮箱</td>
   		<td>${u.userEmail}</td></tr>
        <tr><td>&nbsp;</td></tr>
        <tr><td style="text-align: left; width: 50px;">性别</td>
   		<td>${u.userGender}</td></tr>
   		<tr><td>&nbsp;</td></tr>
   		<tr><td style="text-align: left; width: 50px;">联系电话</td>
   		<td><input type="text" name="user.userPhone"  style="height: 20px" value="${u.userPhone}"/></td></tr> 
        <tr><td>&nbsp;</td></tr>
        <tr><td style="text-align: left; width: 50px;">所在城市</td>
   		 <td><select id="province" name="user.userProvince" onchange="cityName(this.value);">
            <option value="">
            <%--通过数据库中的省份数值显示数组中的值 --%>
            <script type="text/javascript" >
            var n=${u.userProvince}; 
            var provinceName = ["北京市","上海市","天津市","重庆市","河北省","山西省","辽宁省","吉林省","河南省","江苏省","浙江省","安徽省","福建省","江西省","山东省","湖北省","湖南省","广东省","海南省","四川省","贵州省","云南省","陕西省","甘肃省","青海省","黑龙江省","内蒙古自治区","广西壮族自治区","西藏自治区","宁夏回族自治区","新疆维吾尔自治区","台湾省","香港特别行政区","澳门特别行政区","其它"];
             document.write(provinceName[n-1]);
             </script>
            </option>
		</select></td>
			<td><select id="city" name="user.userCity">
            <option value="">
             ${u.userCity}
            </option>
			</select></td></tr>      				                  
  		</table>
		<div align="left"><br><span class="btn"><input style="width: 90px; height: 40px;" type="submit" value="确定修改" onclick="return check1()"/></span></div>
  		</div>
  		</div>
	   </form>
   </div>
